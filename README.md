...
- 记忆系统 V1：按 session 维护短期记忆窗口，并维护面向整个 oncallAgent 的全局偏好记忆。
...
- 记忆调试接口：通过 `/api/chat/memory` 查看全局偏好快照和活动 session 概览。
...
- `docs/generated/` 存放生成类分析和参考文档，例如 AGI-saber 记忆模块分析资料。
